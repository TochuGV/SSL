...
void Declaracion(void){
  ...
  TOKEN tipo_tok = ProximoToken();
  TIPO_DATO tipo_semantico = T_DESCONOCIDO;
  
  // 1. Determinar el TIPO_DATO a partir del TOKEN (Semántica)
  if (tipo_tok == TIPO_CHAR) {
    tipo_semantico = T_CARACTER;
  } else if (tipo_tok == TIPO_FLOAT) {
    tipo_semantico = T_REAL;
  } else if (tipo_tok == TIPO_INT) {
    tipo_semantico = T_ENTERO;
  };
  
  // 2. Consumir el token de tipo (ASDR)
  Match(tipo_tok);
  
  // 3. Procesar la Lista de Identificadores, pasándole el tipo para su registro
  ListaIdentificadores(tipo_semantico); 
  
  // 4. Consumir el final de la declaración
  Match(PUNTOYCOMA);
}
...
void Sentencia(void){
  ...
  switch(tok){
    ...
    case LEER:
      ...
      ListaIdentificadores(T_DESCONOCIDO);
      Match(PARENDERECHO);
      Match(PUNTOYCOMA);
      break;
    case ESCRIBIR:
      ...
      break;   
    case MIENTRAS:
      /* <sentencia> -> MIENTRAS ( <condicion> ) HACER <listaSentencias> FIN_MIENTRAS */
      //SentenciaMientras();
      break;
    case SI:
      /* <sentencia> -> SI ( <condicion> ) ENTONCES <listaSentencias> SINO <listaExpresiones> FIN_SI */
      //Si();
      break;
    case REPETIR:
      /* <sentencia> -> */
...
REG_EXPRESION GenLogico(REG_EXPRESION e1, char* op, REG_EXPRESION e2){
  /* Genera el código para evaluar la condición lógica y almacena el resultado (0 o 1)
     en una variable temporal (TempLog&N). */
  REG_EXPRESION reg;
  ...
  if(!strcmp(op, "=")) strcpy(cadOp, "ComparaIGUAL");
    ...
    strcpy(cadOp, "ComparaIGUAL");
  }

  // Chequear y declarar el temporal (el resultado lógico es siempre T_ENTERO: 0 o 1)
  // NOTA: Asumiendo que Chequear acepta TIPO_DATO.
...
void Condicion(REG_EXPRESION* presul){
  /* <condición> -> <expresión> OP_RELACIONAL <expresión> #gen_logico */
  REG_EXPRESION izq, der;
  char op[TAMLEX];

  Expresion(&izq);
  Match(OP_RELACIONAL);
  strcpy(op, buffer); // El lexema (ej: ">" o "!=") se guarda del buffer
  Expresion(&der);
...
void SentenciaMientras(void)
{
  /* <sentencia_mientras> -> MIENTRAS ( <condición> ) HACER <listaSentencias> FIN_MIENTRAS ; 
     Lógica PE: L1: Evaluar Condición -> Bf L2; Cuerpo; Br L1; L2: */
  REG_EXPRESION cond_res;
  char *etiqueta_inicio;
  char *etiqueta_fin;
  
  etiqueta_inicio = NuevaEtiqueta();
  etiqueta_fin = NuevaEtiqueta();

  // 1. Match MIENTRAS y colocar etiqueta de INICIO (L1)
  Match(MIENTRAS);
  GenerarEtiqueta(etiqueta_inicio); 
  ...
  // 3. Salto Condicional (Bf): Si es Falso, salta a L2
  Generar("Bf", cond_res.nombre, "", etiqueta_fin);
  
  Match(HACER);
  ListaSentencias(); // Cuerpo del ciclo

  // 4. Salto Incondicional (Br): Vuelve a L1 para re-evaluar
  Generar("Br", "", "", etiqueta_inicio);
  
  // 5. Colocar la etiqueta de FIN (L2)
  GenerarEtiqueta(etiqueta_fin);
  
  Match(FIN_MIENTRAS);
  Match(PUNTOYCOMA);
  
  // Liberar memoria
  free(etiqueta_inicio);
  free(etiqueta_fin);
}
...
REG_EXPRESION ProcesarCte(TOKEN clase){
  /* Convierte cadena que representa numero a numero entero/real y construye un registro semantico */
  REG_EXPRESION reg;
  reg.clase = clase;
...
REG_EXPRESION ProcesarId(void){
/* Construye el registro semántico de un ID y busca su tipo en la TS si ya existe. 
     La declaración la realiza la función que la invoca (ListaIdentificadores/Asignar). */
  REG_EXPRESION reg;
  
  reg.clase = ID;
  strcpy(reg.nombre, buffer);
  // Inicializamos el tipo a Desconocido.
  reg.tipo = T_DESCONOCIDO; 

  // Si el ID ya existe, obtenemos su tipo declarado de la TS
  int i = 0;
  while (strcmp("$", TS[i].identifi)) {
    if (!strcmp(reg.nombre, TS[i].identifi)) {
      reg.tipo = TS[i].tipo; 
      break;
    }
    i++;
...
void Mientras(void){
  ...
  sprintf(L1, "L%d", numEtiqueta++);
  sprintf(L2, "L%d", numEtiqueta++);
...
void Repetir(void){
  /* <repetir> -> repetir inicio <listaSentencias> hasta ( <expresion> ) ; */
  REG_EXPRESION cond;
  char L1[10];
  static int numEtiqueta = 1;
...
void ListaIdentificadores(TIPO_DATO tipo_asociado){
  ...
  REG_EXPRESION reg;
  Identificador(&reg);
  // Revisa si recibe un tipo válido, o si recibe T_DESCONOCIDO.

  if (tipo_asociado != T_DESCONOCIDO) { //Si estamos declarando...
    reg.tipo = tipo_asociado;
    Chequear(reg.nombre, reg.tipo);
  } else { //Si no estamos declarando, estamos leyendo.
    Leer(reg);
  };
  ...
  for (t = ProximoToken(); t == COMA; t = ProximoToken()) {
    ...
    if (tipo_asociado != T_DESCONOCIDO) {
      reg.tipo = tipo_asociado;
      Chequear(reg.nombre, reg.tipo);
    } else {
      Leer(reg);
